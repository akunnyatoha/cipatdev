  ...
  <section class="content">
    <div class="container-fluid">
      <div class="row">
        <div class="col-12">
            <div class="card">
                <form action="{{ route('dashboardpage.barang.update', $peminjaman->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="card-body">
                        ...
                        <div class="form-group">
                            <label for="room">Barang</label>
                            <select class="form-control" aria-label="room" id="room" name="room">
                                <option selected disabled>- Pilih Barang -</option>
                                    @foreach ($barangs as $barang)
                                        <option value="{{ $barang->id }}" {{ $peminjaman->barang_id == $barang->id ? 'selected' : '' }}>{{ $barang->name }}</option>
                                    @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="quantity" name="quantity" placeholder="Quantity" value="{{ $peminjaman->quantity }}">
                        </div>
                        <div class="form-group">
